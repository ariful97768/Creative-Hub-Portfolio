...
import { deleteTeamById } from "@/lib/actions/teams-control";
import { Trash } from "lucide-react";
import { useRouter } from "next/navigation";
import Swal from "sweetalert2";

export default function DeleteTeam({ id }: { id: string }) {
  const { user } = useAuth();
    ...
    Swal.fire({
      ...
      .then(async (result) => {
        if (result.isConfirmed) {
          ...
          const result = await deleteTeamById({ id, adminEmail: user.email });
          router.refresh();
