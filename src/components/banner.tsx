...
/**
 * If this section is descendent of home page then pass "Home" as link parameter
 * else pass the parent page name as link parameter
 */
    ...
    <section>
      ...
      >
        ...
        <div className="flex flex-col justify-center gap-3.5 h-full px-14 max-w-360 mx-auto">
          ...
          >
            <Link
              href={`/${link.toLowerCase() === "home" ? "/" : link.toLowerCase().split(" ").join("-")}`}
            >
              {link}
            </Link>
            {subLink && <span className="flex items-center gap-1.5 text-[#317EFE] hover:text-[#317EFE]/90">
              <ChevronRight size={26} />{" "}
              <Link href={`/${subLink.toLowerCase().split(" ").join("-")}`}>{subLink}</Link>
            </span>}
          </motion.p>
        </div>
      </div>
